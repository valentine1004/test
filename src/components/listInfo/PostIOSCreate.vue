<template>
  <va-card :title="`Рекламный пост (IOS)`">
    <div class="form">
      <div class="row">
...
export default {
  ...
  methods: {
    async createPost() {
      ...
      const list = (await database.ref('listInfoIOSTest').once('value')).val()
      const inx = Array.isArray(list) ? list.length : Object.keys(list).length
      const infoAndroidRef = database.ref('listInfoIOSTest')
      ...
      this.loading = false
      this.$router.push({ name: 'listInfoIOS' })
    },
    uploadImages() {
