<template>
  <va-card v-if="promotion !== null" :title="`Promotion: ${cardTitle}`">
    <div class="form">
      <div class="row">
      ...
      <div class="row">
        ...
        <!-- <div class="flex md4 sm4 xs12">
          <va-input
            v-model="promotion.TRANSITION.catalogId"
            :error="!!errors.catalogId.length"
            :error-messages="errors.catalogId"
            @input="errors.catalogId = []"
            label="Catalog ID"
        ...
        <!-- <div class="flex md4 sm4 xs12">
          <va-input
            v-model="promotion.TRANSITION.type"
      ...
      <div class="row">
        <div class="flex md6 sm6 xs12">
          <div class="row">
            <div class="flex md4 sm6 xs12">
              <img v-if="imagesRU.length" class="promotion-icon" :src="imagesRUSrc" />
              <div v-else class="promotion-icon" :style="imageStylesRU"></div>
            </div>
        ...
        <div class="flex md6 sm6 xs12">
          <div class="row">
            <div class="flex md4 sm6 xs12">
              <img v-if="imagesUK.length" class="promotion-icon" :src="imagesUKSrc" />
              <div v-else class="promotion-icon" :style="imageStylesUK"></div>
            </div>
            <div class="flex md8 sm6 xs12">
      ...
      <div class="row">
        <div class="flex md6 sm6 xs12">
          <div class="row">
            <div class="flex md4 sm6 xs12">
              <img v-if="imagesBigRU.length" class="promotion-icon" :src="imagesBigRUSrc" />
              <div v-else class="promotion-icon" :style="imageBigStylesRU"></div>
            </div>
            <div class="flex md8 sm6 xs12">
        ...
        <div class="flex md6 sm6 xs12">
          <div class="row">
            <div class="flex md4 sm6 xs12">
              <img v-if="imagesBigUK.length" class="promotion-icon" :src="imagesBigUKSrc" />
              <div v-else class="promotion-icon" :style="imageBigStylesUK"></div>
            </div>
            <div class="flex md8 sm6 xs12">
      ...
      <div class="row">
        <va-button @click="editPromotion" :disabled="loading">{{ 'Change' }}</va-button>
      </div>
    </div>
  </va-card>
</template>
...
export default {
  ...
  created() {
    ...
    this.promotionRef = database.ref(`actionsV3/${key}`)
    this.promotionRef.once('value').then(this.promotionListener)
  },
  computed: {
    ...mapState({
    ...
    imageStylesRU() {
      return {
        backgroundImage: `url("${this.checkImage(this.promotion.IMAGE.RU)}")`,
    ...
    imageStylesUK() {
      return {
        backgroundImage: `url("${this.checkImage(this.promotion.IMAGE.UK)}")`,
        backgroundSize: 'contain',
        backgroundRepeat: 'no-repeat',
      }
    },
    imageBigStylesRU() {
      return {
        backgroundImage: `url("${this.checkImage(this.promotion.IMAGE_BIG.RU)}")`,
        backgroundSize: 'contain',
        backgroundRepeat: 'no-repeat',
      }
    },
    imageBigStylesUK() {
      return {
        backgroundImage: `url("${this.checkImage(this.promotion.IMAGE_BIG.UK)}")`,
        backgroundSize: 'contain',
        backgroundRepeat: 'no-repeat',
      }
    },
  ...
  methods: {
    promotionListener(snapshot) {
      this.promotion = { ...snapshot.val() }
      if (this.promotion?.TRANSITION?.type) {
        const type = this.transitionTypes.find((el) => el.name === this.promotion?.TRANSITION?.type)
        this.$set(this.transition, 'type', type)
      }
      this.$set(this.transition, 'productId', this.promotion?.TRANSITION?.productId || '')
      this.$set(this.transition, 'productIds', this.promotion?.TRANSITION?.productIds?.join(',') || '')
      this.$set(this.transition, 'catalogId', this.promotion?.TRANSITION?.catalogId || '')
      this.$set(this.transition, 'url', this.promotion?.TRANSITION?.url || '')
    },
    checkImage(url) {
      imageExists(url, (exists) => {
        if (!exists) url = require('@/assets/icons/no-image.png')
      })
      return url
    },
    async editPromotion() {
      ...
      if (this.imagesRU.length !== 0) {
        await this.uploadImage(['IMAGE', 'imagesRU'])
      }
      if (this.imagesUK.length !== 0) {
        await this.uploadImage(['IMAGE', 'imagesUK'])
      }
      if (this.imagesBigRU.length !== 0) {
        await this.uploadImage(['IMAGE_BIG', 'imagesBigRU'])
      }
      if (this.imagesBigUK.length !== 0) {
        await this.uploadImage(['IMAGE_BIG', 'imagesBigUK'])
      }
      ...
      if (this.transition.type !== '') {
        ...
        delete promotion.TRANSITION
      }

      await this.promotionRef.set(promotion, (error) => {
        if (error) {
          console.log(error)
