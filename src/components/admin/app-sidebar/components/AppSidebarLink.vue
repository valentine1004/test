<template>
  <li class="app-sidebar-link" :class="computedLinkClass">
    <router-link
      ...
      :target="target"
    >
      <va-icon v-if="icon" class="app-sidebar-link__item-icon" :style="computedIconStyles" :name="icon" />
      ...
      <va-icon v-if="iconRight" class="app-sidebar-link__item-icon-right" :style="computedIconStyles" :name="iconRight"
        >{{ iconRightContent }}
      </va-icon>
    </router-link>
  </li>
...
export default {
  ...
  computed: {
    ...
    computedLinkStyles() {
      ...
      if (this.icon) style.display = 'flex'
